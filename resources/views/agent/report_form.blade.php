...
@section('title', '通報 - ' . $user->name . ' | ERAPRO Agent')

@push('styles')
<style>
    .report-wrap { max-width:560px; margin:40px auto; padding:0 24px 80px; }
    ...
    <main class="main-content">
        <div class="report-wrap">
            ...
            <div class="report-card">
                <h1>🚨 ユーザーを通報する</h1>
                <p class="report-subtitle">{{ $user->name }} さんについての通報フォームです。</p>
                ...
                <form method="POST" action="{{ route('agent.report_user.store', $user->id) }}">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select id="reason" name="reason" required>
                            ...
                            <option value="迷惑行為" {{ old('reason') === '迷惑行為' ? 'selected' : '' }}>迷惑行為・嫌がらせ</option>
                            <option value="虚偽情報" {{ old('reason') === '虚偽情報' ? 'selected' : '' }}>プロフィールの虚偽情報</option>
                            <option value="無断キャンセル" {{ old('reason') === '無断キャンセル' ? 'selected' : '' }}>無断キャンセル・ドタキャン</option>
